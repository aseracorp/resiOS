...
const NewNetworkButton = ({ fullWidth, refresh }) => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      driver: Yup.string().required(t('Required')),
      parentInterface: Yup.string().when('driver', {
        is: 'macvlan',
        then: Yup.string().required(t('ParentReqForMACVLAN'))
      }),
    }),
    ...
    <>
      <Dialog open={isOpened} onClose={() => setIsOpened(false)}>
        <FormikProvider value={formik}>
          <DialogTitle>{t('management.servapps.networks.list.newNetwork')}</DialogTitle>
          <DialogContent>
            <DialogContentText>
                ...
                <Stack spacing={2} width={'300px'} style={{ marginTop: '10px' }}>
                  ...
                  >
                    ...
                    >
                      ...
                      <MenuItem value="bridge">{t('management.servapps.networks.list.bridge')}</MenuItem>
                      <MenuItem value="host">{t('management.servapps.networks.list.host')}</MenuItem>
                      <MenuItem value="overlay">{t('management.servapps.networks.list.overlay')}</MenuItem>
                      <MenuItem value="macvlan">{t('management.servapps.networks.list.macvlan')}</MenuItem>
                    </Select>
                  </FormControl>
                  ...
                  <TextField
                    ...
                    id="subnet"
                    name="subnet"
                    label={t('management.servapps.networks.list.subnet')}
                  ...
                  {formik.values.driver === 'macvlan' && (
                    <TextField
                      ...
                      label={t('management.servapps.networks.list.parentIf')}
                  ...
                  <CosmosCheckbox
                    name="attachCosmos"
                    label={t('management.servapps.networks.attackNetwork')}
                    formik={formik}
                  />
                </Stack>
              </form>
      ...
      >
        {t('management.servapps.networks.list.newNetwork')}
      </ResponsiveButton>
    </>
  );
};
