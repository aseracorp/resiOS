    ...
    {(config && cronJobs) ? <>
      ...
      <Stack spacing={2}>
        ...
        {Object.keys(cronJobs).map(scheduler => <div>
          <h4>{({
            "Custom": t('management.scheduler.customJobsTitle'),
            "SnapRAID": t('management.scheduler.parityDiskJobsTitle'),
            "__OT__SnapRAID": t('management.scheduler.oneTimeJobsTitle'),
          }[scheduler])}</h4>
          <PrettyTableView 
            ...
            columns={[
              ...
              {
                title: t('management.scheduler.list.scheduleTitle'),
              ...
              },
              {
                ...
                field: (r) => {
                  return <div style={{maxWidth: '400px'}} >{{
                    'running': <Alert icon={<LoadingOutlined />} severity={'info'} color={'info'}>{t('management.scheduler.list.status.runningSince')}{r.LastStarted}</Alert>,
                    'success': <Alert severity={'success'} color={'success'}>{t('management.scheduler.list.status.lastRunFinishedOn')} {r.LastRun}, {t('management.scheduler.list.status.lastRunFinishedOn.duration')} {(new Date(r.LastRun).getTime() - new Date(r.LastStarted).getTime()) / 1000}s</Alert>,
                    'error': <Alert severity={'error'} color={'error'}>{t('management.scheduler.list.status.lastRunExitedOn')} {r.LastRun}</Alert>,
                    'never': <Alert severity={'info'} color={'info'}>{t('management.scheduler.list.status.neverRan')}</Alert>
              ...
              },
              {
                ...
                field: (r) => <>
                  {r.Running ? <Tooltip title="Stop"><IconButton disabled={!r.Cancellable} onClick={() => {
                    ...
                    : <Tooltip title={t('management.scheduler.list.action.run')}><IconButton onClick={() => {
                      API.cron.run(scheduler, r.Name).then(() => {
                        refresh();
                      });
                    }}><PlayCircleOutlined /></IconButton></Tooltip>}
                  <Tooltip title={t('management.scheduler.list.action.logs')}><IconButton onClick={() => {
                    setJobLogs(r);
